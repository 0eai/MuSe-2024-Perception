...
from config import PATH_TO_FEATURES, PATH_TO_LABELS, PARTITION_FILES, HUMOR, PERCEPTION, DATA_FOLDER
...
def load_multi_att_data(task:str,
              paths:Dict[str, str],
              feature:str,
              label_dims: Optional[str],        ## multi-attribute
              normalize: Optional[Union[bool, StandardScaler]] = True,
              save=False,
              ids: Optional[Dict[str, List[str]]] = None,
              data_file_suffix: Optional[str]=None) \
    ...
    is_loaded = False
    data = {'train': {'feature': [], 'label': {label_dim: [] for label_dim in label_dims}, 'meta': {label_dim: [] for label_dim in label_dims}},    ## multi-attribute
    ...
    for label_dim in label_dims:    ## multi-attribute
        data_file_name = f'data_{task}_{feature}_{label_dim + "_" if len(label_dim) > 0 else ""}_{"norm_" if normalize else ""}' \
                        f'{f"_{data_file_suffix}" if data_file_suffix else ""}.pkl'
        data_file = os.path.join(paths['data'], data_file_name)
        ...
        for partition, subject_ids in partition2subject.items():
            ...
            for subject_id in tqdm(subject_ids):
                ...
                data[partition]['meta'][label_dim].extend(metas)        ## multi-attribute
                data[partition]['label'][label_dim].extend(labels)      ## multi-attribute
...
def get_data_feature_partition(partition_file) -> Tuple[Dict[str, str], Dict[str, List[str]]]:
    """
    Reads mappings from subject ids to their partition and vice versa
    :param partition_file: path to the partition file (csv with two columns: id, partition)
    :return: dicts subject2partition, partition2subject
...
def load_custom_subject(feature_path, subject_id, normalizer, label_dim) -> Tuple[List[np.ndarray], np.ndarray, np.ndarray]:
    """
    Loads data for a single subject for the perception task
    :param feature: feature name
    :param subject_id: subject name/ID
...
def load_feature_fusion_data(task:str,
    ...
    print('Constructing data from scratch ...')
    subject2partition, partition2subject = get_data_feature_partition(paths['partition'])
    ...
    for partition, subject_ids in partition2subject.items():
        ...
        for subject_id in tqdm(subject_ids):
            for times, feature_type in enumerate(pt_model_names.keys()):
                ...
                if task == PERCEPTION:
                    features, labels, metas = load_custom_subject(feature_path=feature_path, subject_id=subject_id,
                                                                    normalizer=normalizer, label_dim=label_dim)
