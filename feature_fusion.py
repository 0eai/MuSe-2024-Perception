...
from data_parser import load_data, load_feature_fusion_data
from dataset import MuSeDataset, custom_collate_fn, MuSeFeatureDataset, custom_feature_collate_fn
from eval import evaluate_feature_fusion, calc_auc, calc_pearsons
from model import FeatureFusionModel
from train import train_feature_fusion_model
...
def parse_args():
    ...
    parser.add_argument('--fusion_type', type=str, required=True,
                        help='')
    parser.add_argument('--feature_level', type=str, required=True,
                        help='')
...
def get_model_info(model_path):
    ...
    info['seed'] = int(sub_parts[2].split('_')[2])
...
def main(args):
    ...
    for pt_model_name in args.pt_model_names.values():
        ...
        if not os.path.exists(pt_feature_path):
            pt_model_path = os.path.join(config.BEST_MODEL_FOLDER, task_id, pt_model_name, f'model_{pt_model_info['seed']}.pth')
            print(f"Target pretrained model: {pt_model_path}")
            model = torch.load(pt_model_path, map_location=args.device)
            model.n_to_1 = True

            if 'device' in model.params.__dict__.keys():
                args.device = model_device = model.params.device
            else:
                model.device = model_device = args.device
            ...
            save_extracted_feature(model, model_device, data, pt_feature_path)
...
if __name__ == '__main__':
    ...
    for pt_model_name in args.pt_model_names.values():
        ...
        feature_names.append('multiatt-' if pt_model_info['multi_att'] else '' + '{}-{}'.format(pt_model_info['encoder'], pt_model_info['feature']))

    
    args.log_file_name =  'feature-fusion_{}_[{}]_[{}]_[{}_{}]_{}'.format(datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), '_'.join(feature_names),
                                                 args.d_fc_out, args.lr,args.batch_size, args.seed)
