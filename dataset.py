import torch
from torch.utils.data.dataloader import default_collate
from torch.utils.data.dataset import Dataset
import numpy as np


class MuSeDataset(Dataset):
    def __init__(self, data, partition):
        super(MuSeDataset, self).__init__()
        self.partition = partition
        features, labels = data[partition]['feature'], data[partition]['label']
        metas = data[partition]['meta']
        self.feature_dim = features[0].shape[-1]
        self.n_samples = len(features)

        feature_lens = []
        label_lens = []
        for feature in features:
            feature_lens.append(len(feature))
        label_lens.append(1)

        max_feature_len = np.max(np.array(feature_lens))
        max_label_len = np.max(np.array(label_lens))
        if max_label_len > 1:
            assert(max_feature_len==max_label_len)

        self.feature_lens = torch.tensor(feature_lens)

        features = [np.pad(f, pad_width=((0, max_feature_len-f.shape[0]),(0,0))) for f in features]
        self.features = [torch.tensor(f, dtype=torch.float) for f in features]
        if max_label_len > 1:
            labels = [np.pad(l, pad_width=((0, max_label_len-l.shape[0]),(0,0))) for l in labels]
        self.labels = torch.tensor(np.array(labels), dtype=torch.float)
        self.metas = metas
        pass

    def get_feature_dim(self):
        return self.feature_dim

    def __len__(self):
        return self.n_samples

    def __getitem__(self, idx):
        """

        :param idx:
        :return: feature, feature_len, label, meta with
            feature: tensor of shape seq_len, feature_dim
            feature_len: int tensor, length of the feature tensor before padding
            label: tensor of corresponding label(s) (shape 1 for n-to-1, else (seq_len,1))
            meta: list of lists containing corresponding meta data
        """
        feature = self.features[idx]
        feature_len = self.feature_lens[idx]
        label = self.labels[idx]
        meta = self.metas[idx]

        sample = feature, feature_len, label, meta
        return sample


def custom_collate_fn(data):
    """
    Custom collate function to ensure that the meta data are not treated with standard collate, but kept as ndarrays
    :param data:
    :return:
    """
    tensors = [d[:3] for d in data]
    np_arrs = [d[3] for d in data]
    coll_features, coll_feature_lens, coll_labels = default_collate(tensors)
    np_arrs_coll = np.row_stack(np_arrs)
    return coll_features, coll_feature_lens, coll_labels, np_arrs_coll



##################################################
## Multi-Attribute Funcs
##################################################

class MuSeMultiAttDataset(Dataset):
    def __init__(self, data, partition, label_dims):
        super(MuSeMultiAttDataset, self).__init__()
        self.partition = partition
        features, labels = data[partition]['feature'], data[partition]['label']
        metas = data[partition]['meta']
        self.feature_dim = features[0].shape[-1]
        self.n_samples = len(features)
        self.labels = {}

        feature_lens = []
        label_lens = []
        for feature in features:
            feature_lens.append(len(feature))
        label_lens.append(1)

        max_feature_len = np.max(np.array(feature_lens))
        max_label_len = np.max(np.array(label_lens))
        if max_label_len > 1:
            assert(max_feature_len==max_label_len)

        self.feature_lens = torch.tensor(feature_lens)

        features = [np.pad(f, pad_width=((0, max_feature_len-f.shape[0]),(0,0))) for f in features]
        self.features = [torch.tensor(f, dtype=torch.float) for f in features]
        for label_dim in label_dims:        ## multi-attribute
            if max_label_len > 1:
                    labels[label_dim] = [np.pad(l, pad_width=((0, max_label_len-l.shape[0]),(0,0))) for l in labels[label_dim]]
            self.labels[label_dim] = torch.tensor(np.array(labels[label_dim]), dtype=torch.float)
        self.metas = metas
        pass

    def get_feature_dim(self):
        return self.feature_dim

    def __len__(self):
        return self.n_samples

    def __getitem__(self, idx):
        """

        :param idx:
        :return: feature, feature_len, label, meta with
            feature: tensor of shape seq_len, feature_dim
            feature_len: int tensor, length of the feature tensor before padding
            label: tensor of corresponding label(s) (shape 1 for n-to-1, else (seq_len,1))
            meta: list of lists containing corresponding meta data
        """
        feature = self.features[idx]
        feature_len = self.feature_lens[idx]
        label = {label_dim: label[idx] for label_dim, label in self.labels.items()}     ## multi-attribute
        meta = {label_dim: meta[idx] for label_dim, meta in self.metas.items()}         ## multi-attribute

        sample = feature, feature_len, label, meta
        return sample
    
def custom_multi_att_collate_fn(data):
    """
    Custom collate function to ensure that the meta data are not treated with standard collate, but kept as ndarrays
    :param data:
    :return:
    """
    tensors = [d[:3] for d in data]
    np_arrs = [d[3] for d in data]
    coll_features, coll_feature_lens, coll_labels = default_collate(tensors)

    np_arrs_coll = {}
    for label_dim in coll_labels.keys():
        np_arrs_coll[label_dim] = np.row_stack([np_arr[label_dim] for np_arr in np_arrs])
    return coll_features, coll_feature_lens, coll_labels, np_arrs_coll



##################################################
## Feature Fusion Funcs
##################################################

class MuSeFeatureDataset(Dataset):
    def __init__(self, data, partition):
        super(MuSeFeatureDataset, self).__init__()
        self.partition = partition
        features, labels = data[partition]['feature'], data[partition]['label']
        metas = data[partition]['meta']
        self.feature_dim = features[0].shape[-1]
        self.n_samples = len(features)

        self.features = torch.tensor(np.array(features), dtype=torch.float)
        self.labels = torch.tensor(np.array(labels), dtype=torch.float)
        self.metas = metas
        pass

    def get_feature_dim(self):
        return self.feature_dim

    def __len__(self):
        return self.n_samples

    def __getitem__(self, idx):
        """

        :param idx:
        :return: feature, label, meta with
            feature: tensor of shape seq_len, feature_dim
            label: tensor of corresponding label(s) (shape 1 for n-to-1, else (seq_len,1))
            meta: list of lists containing corresponding meta data
        """
        feature = self.features[idx]
        label = self.labels[idx]
        meta = self.metas[idx]

        sample = feature, label, meta
        return sample


def custom_feature_collate_fn(data):
    """
    Custom collate function to ensure that the meta data are not treated with standard collate, but kept as ndarrays
    :param data:
    :return:
    """
    tensors = [d[:2] for d in data]
    np_arrs = [d[2] for d in data]
    coll_features, coll_labels = default_collate(tensors)
    np_arrs_coll = np.row_stack(np_arrs)
    return coll_features, coll_labels, np_arrs_coll