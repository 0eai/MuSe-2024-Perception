...
from data_parser import load_multi_att_data
from dataset import MuSeMultiAttDataset, custom_multi_att_collate_fn
from eval import evaluate_multi_att, calc_auc, calc_pearsons
from model import MultiAttModel
from train import train_multi_att_model
...
def parse_args():
    ...
    parser.add_argument('--label_dims', type=str, nargs='+', default=["assertiv"])    ## multi-attribute
...
def main(args):
    ...
    data = load_multi_att_data(args.task, args.paths, args.feature, args.label_dims, args.normalize, save=args.cache)
    datasets = {partition:MuSeMultiAttDataset(data, partition, args.label_dims) for partition in data.keys()}
    ...
    if args.eval_model is None:  # Train and validate for each seed
        ...
        for partition,dataset in datasets.items():  # one DataLoader for each partition
            ...
            data_loader[partition] = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                                                                    ...
                                                                    collate_fn=custom_multi_att_collate_fn)
        
        model = MultiAttModel(args)
        print(model) # TODO delete

        print('=' * 50)
        print(f'Training model... [seed {seed}] for at most {args.epochs} epochs')

        val_loss, val_score, best_model_file = train_multi_att_model(args.task, model, data_loader, args.label_dims, args.epochs,
        ...
        if not args.predict:  # run evaluation only if test labels are available
            test_loss, test_score = evaluate_multi_att(args.task, model, data_loader['test'], args.label_dims, loss_fn=loss_fn,
                                                eval_fn=eval_fn, use_gpu=args.use_gpu, device=args.device)
            test_scores.append(test_score)
            print(f'[Test {eval_str}]:  {test_score}')  ## multi-attribute
        ...
        for partition, dataset in datasets.items():  # one DataLoader for each partition
            ...
            data_loader[partition] = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                                                                 ...
                                                                 worker_init_fn=seed_worker,
                                                                 collate_fn=custom_multi_att_collate_fn)
        _, valid_score = evaluate_multi_att(args.task, model, data_loader['devel'], args.label_dims, loss_fn=loss_fn, eval_fn=eval_fn,
                                  use_gpu=args.use_gpu, device=args.device)
        print(f'Evaluating {model_file}:')
        print(f'[Val {eval_str}]: {valid_score}')      ## multi-attribute
        if not args.predict:
            _, test_score = evaluate_multi_att(args.task, model, data_loader['test'], args.label_dims, loss_fn=loss_fn, eval_fn=eval_fn,
                                     use_gpu=args.use_gpu, device=args.device)
    ...
    if args.predict:  # Make predictions for the test partition; this option is set if there are no test labels
        print('Predicting devel and test samples...')  ## multi-attribute
        best_model = torch.load(model_file, map_location=config.device)
        evaluate_multi_att(args.task, best_model, data_loader['devel'], args.label_dims, loss_fn=loss_fn, eval_fn=eval_fn,
                 use_gpu=args.use_gpu, predict=True, prediction_path=args.paths['predict'],
                 filename='predictions_devel.csv', device=args.device)
        evaluate_multi_att(args.task, best_model, data_loader['test'], args.label_dims, loss_fn=loss_fn, eval_fn=eval_fn,
...
if __name__ == '__main__':
    ...
    ## list of string parsing for compatibility with Sweep
    args.label_dims = args.label_dims[0].lstrip('[\'').rstrip('\']').split('-')
    for label_dim in args.label_dims:
        assert (label_dim in config.PERCEPTION_LABELS)

    args.log_file_name =  'MultiAtt_{}_{}_[{}]_[{}_{}_{}_{}]_[{}_{}]_{}'.format(args.encoder, datetime.now(tz=tz.gettz()).strftime("%Y-%m-%d-%H-%M"), args.feature.replace(os.path.sep, "-"),
                                                 args.model_dim, args.encoder_n_layers, args.rnn_bi, args.d_fc_out, args.lr,args.batch_size, args.seed)
    
    # args.log_file_name = 'MultiAtt_RNN_2024-06-18-19-19_[vit-fer]_[512_2_False_64]_[0.001_256]_101' + '_' + 'leader_like' + f'_{args.epochs}' # Model Reproduction
    ...
    task_id = args.task if args.task != PERCEPTION else os.path.join(args.task, wandb.run.project.split('_')[-1].lower())
    print(f'{task_id=}')
